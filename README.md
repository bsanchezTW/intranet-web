...
| Formato | Tablas |
|---------|--------|
| **6 dígitos aleatorios** (100000–999999) | `users`, `vacation_requests`, `expense_requests` |
| **4 dígitos aleatorios** (1111–9999) | `work_areas`, `support_tickets` |
| **integer IDENTITY** (1, 2, 3…) | `news_articles`, `events`, `applications`, `courses`, `vacation_periods`, `claude_conversations`, `vacation_balance_adjustments`, `lunch_menu`, `public_holidays`, `documents`, `other_documents`, `study_materials`, `event_photos`, `linkedin_posts`, `user_course_progress`, `cost_centers` |
| **bigint IDENTITY** (1, 2, 3…) | `questions`, `question_options`, `change_log`, `claude_messages`, `ticket_replies`, `expense_request_items`, `expense_request_attachments` |

El trigger `BEFORE INSERT` `trg_six_digit_id` asigna el número en `users`, `vacation_requests` y `expense_requests` si el `INSERT` no trae `id`. `work_areas` y `support_tickets` usan `trg_four_digit_id`. El catálogo y los hilos usan `GENERATED BY DEFAULT AS IDENTITY`. No hay generador de IDs de usuario en Node: el registro y RRHH hacen `INSERT … RETURNING id`.
...
Las noticias también se pueden abrir por slug. Tras aplicar el SQL, conviene comprobar: `users`/`vacation_requests` con `MIN(id) >= 100000`; `work_areas` y `support_tickets` en 1111–9999 con `trg_four_digit_id`; catálogo e hijas con IDENTITY (bigint en quiz/logs/hilos); FKs (`work_area_id`, `ticket_id`, `course_id`, `question_id`, `vacation_period_id`) coherentes en ambos schemas.
