...
El trigger `BEFORE INSERT` `trg_six_digit_id` asigna el número en `users` y `vacation_requests`, y `trg_eight_digit_id` en `expense_requests`, si el `INSERT` no trae `id`. `work_areas` y `support_tickets` usan `trg_four_digit_id`. El catálogo y los hilos usan `GENERATED BY DEFAULT AS IDENTITY`. No hay generador de IDs de usuario en Node: el registro y RRHH hacen `INSERT … RETURNING id`.
