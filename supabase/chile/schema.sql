-- Schema Chile. Extra vs Perú: linkedin_posts.
-- El catálogo de Aplicaciones vive en shared.applications
-- (supabase/shared/schema.sql); chile.applications es una vista.
-- La galería vive en shared.events; chile.events es una vista.
...
CREATE UNIQUE INDEX IF NOT EXISTS users_national_id_unique_idx
  ON chile.users (national_id) WHERE national_id IS NOT NULL;
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE con.conname = 'work_areas_manager_fk'
      AND c.relname = 'work_areas' AND n.nspname = 'chile'
  ) THEN
    ALTER TABLE chile.work_areas
      ADD CONSTRAINT work_areas_manager_fk
      FOREIGN KEY (manager_user_id) REFERENCES chile.users(id) ON DELETE SET NULL;
  END IF;
END
$do$;
CREATE INDEX IF NOT EXISTS idx_work_areas_manager
  ON chile.work_areas (manager_user_id) WHERE manager_user_id IS NOT NULL;
...
ALTER TABLE chile.users ADD COLUMN IF NOT EXISTS personal_email character varying(150);
ALTER TABLE chile.users ADD COLUMN IF NOT EXISTS work_phone numeric;
...
CREATE INDEX IF NOT EXISTS idx_claude_conversations_user_id
  ON chile.claude_conversations (user_id);
CREATE INDEX IF NOT EXISTS idx_claude_conversations_updated_at
  ON chile.claude_conversations (updated_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_claude_messages_conversation_id
  ON chile.claude_messages (conversation_id);
...
CREATE TABLE IF NOT EXISTS chile.claude_user_settings (
  user_id integer PRIMARY KEY REFERENCES chile.users(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS chile.documents (
  ...
  work_area_id integer REFERENCES chile.work_areas(id) ON DELETE CASCADE,
  doc_kind character varying(20)
);
CREATE INDEX IF NOT EXISTS idx_documents_area_kind
  ON chile.documents (work_area_id, doc_kind);
...
-- ===========================================================
CREATE TABLE IF NOT EXISTS chile.cost_centers (
  id integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
CREATE UNIQUE INDEX IF NOT EXISTS cost_centers_code_unique_idx
  ON chile.cost_centers (UPPER(TRIM(code)));
CREATE INDEX IF NOT EXISTS idx_cost_centers_active
  ON chile.cost_centers (active) WHERE active = true;
...
CREATE TABLE IF NOT EXISTS chile.user_cost_centers (
  user_id integer NOT NULL REFERENCES chile.users(id) ON DELETE CASCADE,
  cost_center_id integer NOT NULL REFERENCES chile.cost_centers(id) ON DELETE CASCADE,
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  PRIMARY KEY (user_id, cost_center_id)
);
CREATE INDEX IF NOT EXISTS idx_user_cost_centers_center
  ON chile.user_cost_centers (cost_center_id);
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE t.typname = 'expense_request_kind' AND n.nspname = 'chile'
  ) THEN
    CREATE TYPE chile.expense_request_kind AS ENUM ('rendicion','fondos');
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_type t JOIN pg_namespace n ON n.oid = t.typnamespace
    WHERE t.typname = 'expense_request_status' AND n.nspname = 'chile'
  ) THEN
    CREATE TYPE chile.expense_request_status AS ENUM
...
CREATE TABLE IF NOT EXISTS chile.expense_requests (
  ...
  manager_user_id integer REFERENCES chile.users(id) ON DELETE SET NULL,
  manager_reviewed_by integer REFERENCES chile.users(id) ON DELETE SET NULL,
  manager_reviewed_at timestamp with time zone,
  manager_notes text,
  finance_reviewed_by integer REFERENCES chile.users(id) ON DELETE SET NULL,
  ...
  cost_center_id integer REFERENCES chile.cost_centers(id) ON DELETE SET NULL,
  ...
  bank_code character varying(3) REFERENCES chile.banks(code) ON UPDATE CASCADE,
...
CREATE TABLE IF NOT EXISTS chile.expense_request_items (
  id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  request_id integer NOT NULL REFERENCES chile.expense_requests(id) ON DELETE CASCADE,
  item_date date,
...
CREATE INDEX IF NOT EXISTS idx_expense_items_request
  ON chile.expense_request_items (request_id, sort_order);

CREATE TABLE IF NOT EXISTS chile.expense_request_attachments (
  id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  request_id integer NOT NULL REFERENCES chile.expense_requests(id) ON DELETE CASCADE,
  item_id bigint REFERENCES chile.expense_request_items(id) ON DELETE SET NULL,
  name character varying(200) NOT NULL,
  url text NOT NULL,
  public_id text,
  created_at timestamp with time zone NOT NULL DEFAULT now()
...
CREATE INDEX IF NOT EXISTS idx_expense_attachments_request
  ON chile.expense_request_attachments (request_id);
CREATE INDEX IF NOT EXISTS idx_expense_attachments_item
  ON chile.expense_request_attachments (item_id)
  WHERE item_id IS NOT NULL;

-- Galería: tabla en shared.events (supabase/shared/schema.sql).
-- Pública para ambos países; privada solo si country_code es CL.
...
CREATE TABLE IF NOT EXISTS chile.question_options (
  id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  question_id bigint REFERENCES chile.questions(id) ON DELETE CASCADE,
  text text NOT NULL,
...
CREATE TABLE IF NOT EXISTS chile.ticket_replies (
  id integer NOT NULL,
  ticket_id integer NOT NULL REFERENCES chile.support_tickets(id) ON DELETE CASCADE,
...
ALTER TABLE chile.cost_centers ENABLE ROW LEVEL SECURITY;
ALTER TABLE chile.user_cost_centers ENABLE ROW LEVEL SECURITY;
ALTER TABLE chile.expense_requests ENABLE ROW LEVEL SECURITY;
ALTER TABLE chile.expense_request_items ENABLE ROW LEVEL SECURITY;
ALTER TABLE chile.expense_request_attachments ENABLE ROW LEVEL SECURITY;
ALTER TABLE chile.banks ENABLE ROW LEVEL SECURITY;
ALTER TABLE chile.user_bank_accounts ENABLE ROW LEVEL SECURITY;
...
CREATE OR REPLACE FUNCTION chile.next_four_digit_id(p_table text)
RETURNS integer
LANGUAGE plpgsql
AS $fn$
DECLARE
...
BEGIN
  ...
  FOR i IN 1..80 LOOP
    ...
    END IF;
  END LOOP;
  RAISE EXCEPTION 'No fue posible generar un ID de 4 dígitos para chile.%', p_table;
END;
$fn$;

CREATE OR REPLACE FUNCTION chile.assign_four_digit_id()
RETURNS trigger
LANGUAGE plpgsql
AS $fn$
BEGIN
  IF NEW.id IS NULL OR NEW.id < 1111 OR NEW.id > 9999 THEN
    NEW.id := chile.next_four_digit_id(TG_TABLE_NAME);
  END IF;
  RETURN NEW;
END;
$fn$;
...
CREATE OR REPLACE FUNCTION chile.assign_ticket_reply_id()
RETURNS trigger
LANGUAGE plpgsql
AS $fn$
BEGIN
  ...
  END IF;
  PERFORM 1 FROM chile.support_tickets WHERE id = NEW.ticket_id FOR UPDATE;
  IF NEW.id IS NULL OR NEW.id <= 0 THEN
    SELECT COALESCE(MAX(id), 0) + 1
      INTO NEW.id
      FROM chile.ticket_replies
...
DROP TRIGGER IF EXISTS trg_ticket_reply_id ON chile.ticket_replies;
CREATE TRIGGER trg_ticket_reply_id
  BEFORE INSERT ON chile.ticket_replies
  FOR EACH ROW EXECUTE FUNCTION chile.assign_ticket_reply_id();
...
BEGIN
  ...
  FOREACH tname IN ARRAY four_digit_tables LOOP
    ...
    EXECUTE format(
      'CREATE TRIGGER trg_four_digit_id BEFORE INSERT ON %I.%I FOR EACH ROW EXECUTE FUNCTION chile.assign_four_digit_id()',
      p_schema, tname
    );
  END LOOP;
...
ALTER FUNCTION chile.assign_ticket_reply_id() SET search_path = chile, pg_temp;
...
REVOKE ALL ON FUNCTION chile.assign_ticket_reply_id() FROM PUBLIC;
...
GRANT EXECUTE ON FUNCTION chile.assign_ticket_reply_id() TO intranet_chile;
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'service_role') THEN
    ...
    GRANT EXECUTE ON FUNCTION chile.assign_ticket_reply_id() TO service_role;
